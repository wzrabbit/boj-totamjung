...
const getRandomDefenseResult = async (searchQuery) => {
  try {
    const response = await fetch(
      `https://solved.ac/api/v3/search/problem?query=${searchQuery}&sort=random`
    );
  ...
  if (message.msg === 'openOption') {
    ...
    chrome.storage.sync.get('timer', (loaded) => {
      ...
      if (message.msg === 'setTimer') {
        loaded.problem = parseInt(
          sender.url.split('problem/')[1].match(/^\d+/g)[0]
        );
        loaded.expire =
          new Date().getTime() + loaded.hour * 3600000 + loaded.minute * 60000;
    ...
    chrome.storage.sync.get('query', (loaded) => {
      ...
      if (
        ...
        no === -1 &&
        loaded.selectedNo &&
        !loaded[loaded.selectedNo].isEmpty
      ) {
      ...
      getRandomDefenseResult(searchQuery).then((requestResult) => {
    ...
    chrome.storage.local.get('queryLog', (loaded) => {
      ...
      try {
        for (
          let i = Math.max(0, queryLog.length - 300);
          i < queryLog.length;
          i++
        ) {
      ...
      try {
        for (let i = 0; i < queryLog.length; i++) {
          ...
          if (
            'no' in queryLog[i] &&
            queryLog[i].no >= 1000 &&
            Number.isInteger(queryLog[i].no)
          ) {
          ...
          if (
            'tier' in queryLog[i] &&
            Number.isInteger(queryLog[i].tier) &&
            queryLog[i].tier >= -1 &&
            queryLog[i].tier <= 31
          ) {
      ...
      for (let i = 0; i <= 9; i++) {
        try {
          if (typeof loaded === 'object' && loaded[i]) {
            ...
            if ('title' in loaded[i]) data[i].title = loaded[i].title;
