...
const createSlot = () => {
  chrome.runtime.sendMessage({ msg: 'getSlotData' }, (res) => {
    ...
    [1, 2, 3, 4, 5, 6, 7, 8, 9, 0].forEach((i) => {
      slotHTML += `
        <label>
            <input class="hidden slot-radio${
              res[i].isEmpty ? '' : ' ready'
            }" type="radio" name="slot" id="slot-radio-${i}"/>
...
const showInfoAndSaveNo = (no) => {
  chrome.runtime.sendMessage({ msg: 'getSlotData' }, (res) => {
    ...
    chrome.runtime.sendMessage({ msg: 'saveSlotNo', no: no }, (res) => {
      if (res.result === 'FAIL')
        alert(
          '죄송합니다. 데이터 저장 도중 문제가 발생했습니다. 페이지를 새로고침 해 주세요.'
        );
    });
  });
};
...
const deleteQuery = () => {
  chrome.runtime.sendMessage({ msg: 'getSlotData' }, (res) => {
    ...
    slotQuery.innerHTML = `<span style="color: var(--empty-color)">(비어 있음)</span>`;
    document
      .querySelector(`#slot-radio-${res.selectedNo}`)
...
const checkValid = (mode) => {
  if (rdTitle.value.length > 30)
    return {
      result: 'FAIL',
      errorMessage: '추첨 이름은 30자 이하여야 합니다.',
    };
  ...
  if (mode === 'left') {
    if (rdNickName.value.trim() !== '') {
      if (!/^[A-z0-9]+$/.test(rdNickName.value) && rdNickName.value !== '') {
        ...
        return {
          result: 'FAIL',
          errorMessage: '닉네임은 알파벳과 숫자로만 구성되어야 합니다.',
        };
      }
    ...
    if (
      isNaN(rdMaxSolved.value) ||
      isNaN(rdMinSolved.value) ||
      rdMaxSolved.value < 0 ||
      rdMinSolved.value < 0
    ) {
      ...
      return {
        result: 'FAIL',
        errorMessage: '맞은 사람 수는 0 이상의 정수여야 합니다.',
      };
    }
    ...
    if (
      rdMinSolved.value !== '' &&
      rdMaxSolved.value !== '' &&
      rdMinNumberValue > rdMaxNumberValue
    ) {
      ...
      return {
        result: 'FAIL',
        errorMessage: '맞은 사람 수의 좌변은 우변보다 클 수 없습니다.',
      };
    }
    ...
    if (rdMinNumberValue > 100000000 || rdMaxNumberValue > 100000000) {
      ...
      return {
        result: 'FAIL',
        errorMessage: '맞은 사람 수는 100,000,000 명을 넘을 수 없습니다.',
      };
    }
  } else {
...
const saveQuery = (mode) => {
  chrome.runtime.sendMessage({ msg: 'getSlotData' }, (res) => {
    ...
    if (title.trim() === '') title = `추첨 ${res.selectedNo}`;
    ...
    if (mode === 'left') {
      ...
      if (rdMinSolved.value.trim() !== '' || rdMaxSolved.value.trim() !== '')
        query += `solved:${rdMinSolved.value.replaceAll(
          ' ',
          ''
        )}..${rdMaxSolved.value.replaceAll(' ', '')} `;
...
searchInput.addEventListener('focusout', (e) => {
  setTimeout(() => {
    if (
      ![searchInput, mainDiv, keywordDiv, searchDiv].includes(e.relatedTarget)
    ) {
...
const getSearchResult = (keyword) => {
  ...
  for (let i = 1; i <= Object.keys(db).length; i++) {
    let word = [
      db[i].name,
      db[i].tag,
      ...(db[i].alias.length > 0 ? db[i].alias : []),
    ].map((x) => trimWord(x));
    let found = false;
...
searchInput.addEventListener('keyup', (e) => {
  ...
  searchInput.style.width = `${(
    getFakeWidth(searchInput.value) + 20
  ).toString()}px`;
});
