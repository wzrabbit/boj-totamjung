      ...
      if (found || keyword === '') {
        printer += `
                <div class="algorithm-block">
                    <label>
                        <span>${db[i].name}</span>
                        <input class="hidden checkbox" type="checkbox" no="${i}" ${
          algorithm.has(i) ? 'checked' : ''
        }/>
...
function makePoolSearchListener() {
  ...
  poolSearcher.addEventListener('keydown', (e) => {
    if (e.keyCode === 27) poolSearcher.value = '';
  });
  ...
  chrome.storage.sync.get(['settings'], (loaded) => {
    ...
    ['predict', 'lock', 'theme', 'font'].forEach((name) => {
      let radioElement = document.querySelector(
        `.radio[name="${name}"].radio[value="${loaded[name]}"]`
      );
  ...
  chrome.storage.sync.get(['timer'], (loaded) => {
    ...
    if (element === hour) {
      if (
        ...
        hour.value > 8 ||
        hour.value < 0 ||
        hour.value.toString().length >= 2 ||
        hour.value.toString().includes('-')
      )
        hour.value = loaded.hour;
      ...
      if (
        ...
        minute.value > 59 ||
        minute.value < 0 ||
        minute.value.toString().length >= 3 ||
        minute.value.toString().includes('-') ||
